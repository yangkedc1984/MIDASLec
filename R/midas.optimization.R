midas.optimization <- function(rv,optionsmidas,weights,nInitialCond=5) {
...
for (k in 1:Ns){
  V[k,1]  <-  midas.objfn(initialTargetVectors[k,],y,X,1,weights)
}          
...
for (dt in 1:nInitialCond){
  est[[dt]] <-  suppressWarnings(optimx::optimx(BestInitialCond[dt,],midas.objfn,y=y,X=X,OUT=1,weights=weights,method=c("Nelder-Mead")))
}
val <- NULL
...
objfn.MIDAS <- min(val)
cond.val.MIDAS <- midas.objfn(coeff.MIDAS,y,X,2,weights)
return(list(coeff.MIDAS=coeff.MIDAS, objfn.MIDAS=objfn.MIDAS,cond.val.MIDAS=cond.val.MIDAS))
}


midas.objfn <- function(theta,y,X,OUT,weights) {
  # objective fn for the MIDAS estimation
  # MIDAS vol
  val <- theta[1]+theta[2]*midas.filter(theta[-c(1,2)],X,weights)
  # Obj Fn.
  Q  <- sum((y - val)^2)*1000
  if ((Q == Inf)  || (is.complex(Q))) {
    Q <- 1e+10000
  }
  if (OUT == 1){
    return(Q)
